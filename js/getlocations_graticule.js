...
// Ported to Drupal by Bob Hutchinson
//

(function ($) {

  Drupal.behaviors.getlocations_graticule = {
    attach: function() {

      Drupal.getlocations.Graticule = (function() {
        ...
        function _bestTextColor(overlay) {
          ...
          var lc = Drupal.getlocations_settings[Drupal.getlocations_grat.key].graticule_lightcolor;
          var dc = Drupal.getlocations_settings[Drupal.getlocations_grat.key].graticule_darkcolor;
        ...
        GraticuleOverlay.prototype.draw = function() {
          ...
          for (var lo = l; lo < r; lo += dLng) {
            ...
            if (Drupal.getlocations_settings[Drupal.getlocations_grat.key].graticule_label > 0) {
              // default decimal
              var lb = lo.toFixed(gridPrecision(dLng));
              // label type
              if (Drupal.getlocations_settings[Drupal.getlocations_grat.key].graticule_label == 2) {
                // dms from geo.js
                lb = Drupal.getlocations.geo.dd_to_dms_lng(lo);
              }
              else if (Drupal.getlocations_settings[Drupal.getlocations_grat.key].graticule_label == 3) {
                // sexagesimal
                lb = this.decToSex(lo);
              }
          ...
          for (; b <= t; b += dLat) {
            ...
            if (Drupal.getlocations_settings[Drupal.getlocations_grat.key].graticule_label > 0) {
              ...
              if (Drupal.getlocations_settings[Drupal.getlocations_grat.key].graticule_label == 2) {
                // dms from geo.js
                lba = Drupal.getlocations.geo.dd_to_dms_lat(b);
              }
              else if (Drupal.getlocations_settings[Drupal.getlocations_grat.key].graticule_label == 3) {
                // sexagesimal
                lba = this.decToSex(b);
